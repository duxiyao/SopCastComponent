package opengl.study.demos._7light;
...
public class DiffuseRender implements GLSurfaceView.Renderer {

    private static final float ONE_SEC = 1000.0f; // 1 second

    private Context context;
    private DiffuseCube   cube;
    private long lastTimeMillis = 0L;

    public DiffuseRender(Context context) {
    ...
    public void onSurfaceCreated(GL10 gl10, EGLConfig eglConfig) {
        ...
        ShaderProgram shader = new ShaderProgram(
                ShaderUtils.readShaderFileFromRawResource(context, R.raw.diffuse_vertex_shader),
                ShaderUtils.readShaderFileFromRawResource(context, R.raw.diffuse_frag_shader)
        );

        int textureName = TextureUtils.loadTexture(context, R.mipmap.dungeon_01);

        cube = new DiffuseCube(shader);
