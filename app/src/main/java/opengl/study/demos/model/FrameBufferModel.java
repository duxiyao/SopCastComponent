package opengl.study.demos.model;
...
public class FrameBufferModel {
    ...
    public FrameBufferModel(String name, ShaderProgram shader, float[] vertices, short[] indices) {
        this.name = name;
        this.shader = shader;
    ...
    public void draw() {
        ...
        GLES20.glActiveTexture(GLES20.GL_TEXTURE0);                 // 1번 텍스처 슬롯을 activate 한다.
        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, textureName);    // 1번 텍스처 슬롯에 생성한 텍스처를 바인딩한다.
        shader.setUniformi("u_Texture", 0);                         // 유니폼 u_Texture에는 activate한 텍스처슬롯번호를 알려준다.
    ...
    protected void draw(int texture) {
        shader.begin();

        GLES20.glActiveTexture(GLES20.GL_TEXTURE0);                 // 1번 텍스처 슬롯을 activate 한다.
        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, texture);    // 1번 텍스처 슬롯에 생성한 텍스처를 바인딩한다.
        shader.setUniformi("u_Texture", 0);                         // 유니폼 u_Texture에는 activate한 텍스처슬롯번호를 알려준다.
    ...
    protected void drawbf(int texture){
