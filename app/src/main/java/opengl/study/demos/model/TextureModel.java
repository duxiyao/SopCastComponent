...
public class TextureModel {
    ...
    public TextureModel(String name, ShaderProgram shader, float[] vertices, short[] indices) {
        this.name = name;
        this.shader = shader;
    ...
    public void draw(long dt) {
        ...
        GLES20.glActiveTexture(GLES20.GL_TEXTURE1);                 // 1号纹理槽进行activate。
        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, textureName);    // 在1号纹理槽中生成的纹理进行绑定。
        shader.setUniformi("u_Texture", 1);           // 制服u Texture告诉你activate的纹理槽号码。
